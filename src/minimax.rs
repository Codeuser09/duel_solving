...
pub fn minimax (board: &Board, info_matrix: &InfoMatrix, depth: i32, is_white: bool) -> i32 {
    ...
    return if is_white == true {
        ...
        for legal_move in get_legal_moves(&board, &info_matrix, is_white) {
            ...
            let eval = minimax(&new_board, &new_info_matrix, depth - 1, false);
        ...
        for legal_move in get_legal_moves(&board, &info_matrix, is_white) {
            ...
            let mut new_info_matrix = info_matrix.clone();
            make_move(&mut new_board, &mut new_info_matrix, &is_white, &legal_move, );
            let eval = minimax(&new_board, &new_info_matrix, depth - 1, true);
